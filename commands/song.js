...
async function getAnabotDownloadByUrl(youtubeUrl) {
    const apikey = 'freeApikey';
    const apiUrl = `https://anabot.my.id/api/download/ytmp3?url=${encodeURIComponent(youtubeUrl)}&apikey=${encodeURIComponent(apikey)}`;
    const res = await tryRequest(() => axios.get(apiUrl, AXIOS_DEFAULTS));
...
async function songCommand(sock, chatId, message) {
    try {
        ...
        await sock.sendMessage(chatId, {
            ...
            caption: `🎵 Downloading: *${video.title || 'Song'}*\n⏱ Duration: ${video.timestamp || 'Unknown'}`
        }, { quoted: message });

        // Try Anabot primary, then Keith, then Yupra fallback
        let audioData;
        try {
            // 1) Primary: Anabot API
            audioData = await getAnabotDownloadByUrl(video.url);
        } catch (e0) {
            try {
                // 2) Secondary: Keith
                audioData = await getKeithDownloadByUrl(video.url);
            } catch (e1) {
                // 3) Fallback: Yupra
